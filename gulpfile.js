...
gulp.task('makepot', function () {
  return gulp.src(['**/*.php'])
    ...
    .pipe(wpPot({
      ...
      team: ' <marconte@thatmuch.fr>'
    }))
    .pipe(gulp.dest('languages/goodmornigkorea.pot'))
    .pipe(browserSync.reload({stream:true}));
});

gulp.task('browserSync', function() {
  browserSync({
    proxy: 'localhost/goodmorningkorea',
    port: 8080,
    open: true,
    notify: false
  });
